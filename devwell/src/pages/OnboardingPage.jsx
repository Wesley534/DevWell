...
export const OnboardingPage = () => {
  ...
  useEffect(() => {
    const checkProfile = async () => {
      ...
      try {
        ...
        toast.error('Network error while checking profile');
      }
    };
  ...
  const validateStep = () => {
    if (step === 1) {
      return formData.nickname && formData.timezone;
    }
    if (step === 2) {
      return formData.work_hours_start && formData.work_hours_end && formData.coding_style;
    }
    if (step === 3) {
      return formData.wellness_goals.length > 0;
    }
    if (step === 4) {
      return formData.diet_preference && formData.reminder_frequency;
    }
    return true; // Step 5 (age, weight) is optional
  };
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      ...formData,
      wellness_goals: formData.wellness_goals.join(','), // Convert array to comma-separated string
    ...
    try {
      console.log('Sending request to /api/profile/onboarding...');
      ...
      if (response.ok) {
        console.log('Onboarding completed:', await response.json());
